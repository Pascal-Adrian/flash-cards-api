...
@TagRouter.get("/", response_model=list[Tag])
...
async def get_tag(id: int, db=Depends(get_db)):
...
async def create_tag(tag_create: TagCreate, db=Depends(get_db)):
...
async def update_tag(id: int, tag_update: TagCreate, db=Depends(get_db)):
    result = await tag.update(db, id, tag_update)
    return result


@TagRouter.delete("/{id}", response_model=None)
async def delete_tag(id: int, db=Depends(get_db)):
